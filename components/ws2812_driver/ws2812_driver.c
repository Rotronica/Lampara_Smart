...
esp_err_t ws2812_driver_init(const ws2812_driver_config_t *config)
{
    ...
    if (!config || config->num_leds <= 0) return ESP_ERR_INVALID_ARG;
    if (s_drv) ws2812_driver_deinit();

    s_drv = calloc(1, sizeof(ws2812_driver_t));
    if (!s_drv) return ESP_ERR_NO_MEM;
...
{
    if (!s_drv || !s_drv->initialized) return;
    if (index < 0 || index >= s_drv->num_leds) return;
...
{
    if (!s_drv) return;
    if (s_drv->rmt_encoder) rmt_del_encoder(s_drv->rmt_encoder);
    if (s_drv->rmt_channel) rmt_del_channel(s_drv->rmt_channel);
    if (s_drv->buffer) free(s_drv->buffer);
    free(s_drv);
    s_drv = NULL;
}