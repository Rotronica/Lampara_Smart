...
esp_err_t led_controller_init(const led_controller_config_t *config)
{
    ...
    if (!s_led) return ESP_ERR_NO_MEM;
...
esp_err_t led_controller_set_color(int index, uint8_t r, uint8_t g, uint8_t b)
{
    ...
    if (index < 0 || index >= s_led->num_leds) return ESP_ERR_INVALID_ARG;
...
void led_controller_hsv2rgb(uint32_t h, uint32_t s, uint32_t v, uint8_t *r, uint8_t *g, uint8_t *b)
{
    ...
    uint32_t rv, gv, bv;
    switch (i) {
        case 0: rv = rgb_max; gv = rgb_min + rgb_adj; bv = rgb_min; break;
        case 1: rv = rgb_max - rgb_adj; gv = rgb_max; bv = rgb_min; break;
        case 2: rv = rgb_min; gv = rgb_max; bv = rgb_min + rgb_adj; break;
        case 3: rv = rgb_min; gv = rgb_max - rgb_adj; bv = rgb_max; break;
        case 4: rv = rgb_min + rgb_adj; gv = rgb_min; bv = rgb_max; break;
        default: rv = rgb_max; gv = rgb_min; bv = rgb_max - rgb_adj; break;
    }
